...
void SystemCoreClockUpdate(void) {
	...
	SystemCoreClock =
		__SYSTEM_CLOCK_MAX >> (NRF_CLOCK_NS->HFCLKCTRL & (CLOCK_HFCLKCTRL_HCLK_Msk));
#else
	SystemCoreClock =
 ...
 * Nordic SPU should handle Secure Attribution tasks */
	...
	/* Workaround for Errata 97 "ERASEPROTECT, APPROTECT, or startup problems" found at the
	...
	/* Trimming of the device. Copy all the trimming values from FICR into the target addresses.
	 Trim until one ADDR is not initialized. */
	uint32_t index = 0;
	for (index = 0; index < 32ul && NRF_FICR_S->TRIMCNF[index].ADDR != (uint32_t*)0xFFFFFFFFul;
		index++) {
...
#pragma diag_default = Pa082
	...
	/* errata 64 must be before errata 42, as errata 42 is dependant on the changes in errata
	 * 64*/
	/* Workaround for Errata 64 "VREGMAIN has invalid configuration when CPU is running at 128
	   MHz" found at the Errata document for your device located at
	...
	/* Workaround for Errata 42 "Reset value of HFCLKCTRL is invalid" found at the Errata
	...
	/* Workaround for Errata 49 "SLEEPENTER and SLEEPEXIT events asserted after pin reset" found
	...
	/* Workaround for Errata 55 "Bits in RESETREAS are set when they should not be" found at the
	   Errata document for your device located at https://infocenter.nordicsemi.com/index.jsp */
	if (nrf53_errata_55()) {
		if (NRF_RESET_S->RESETREAS & RESET_RESETREAS_RESETPIN_Msk) {
	...
	/* Workaround for Errata 69 "VREGMAIN configuration is not retained in System OFF" found at
...
/* Enable Trace functionality. If ENABLE_TRACE is not defined, TRACE pins will be used as GPIOs (see
   Product Specification to see which ones). */
	 ...
	 * If only the secure code should control FPU power state these registers should be
	 * configured accordingly in the secure application code. */
	SCB->NSACR |= (3UL << 10);
