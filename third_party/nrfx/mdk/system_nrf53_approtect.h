...
/* Function that handles firmware-driven enabling or disabling of APPROTECT on devices where it is
   supported. If ENABLE_APPROTECT is defined, the FW will lock the fw branch of the APPROTECT
   mechanism, preventing it from being opened. If ENABLE_APPROTECT_USER_HANDLING is defined, the FW
   will not write to the fw branch of the APPROTECT mechanism. This allows later stages of the fw to
   handle APPROTECT, for example to implement authenticated debug. Otherwise, the fw branch state is
   loaded from UICR.
...
static inline void nrf53_handle_approtect(void) {
	...
	/* Do nothing, allow user code to handle APPROTECT. Use this if you want to enable
	 * authenticated debug. */
...
/* Secure APPROTECT is only available for Application core. */
	...
	/* Do nothing, allow user code to handle SECURE APPROTECT. Use this if you want to enable
	 * authenticated debug. */
